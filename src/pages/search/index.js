...
class Search extends React.Component {
	...
	getSearchDetails = event => {
		...
		post('getteammatchesbyleague', payload).then(res => {
			console.log({ res });
	...
	getMatchDetails() {
		...
		post('matchsForSearch', payloadteam).then(res => {
			if (res.data.success == 1) {
				this.setState({
					matchList: res.data.matches,
					// fixtures: res.data.fixtures,
	...
	getNewsDetails() {
		...
		post('newsForTeam', payloadnews).then(res => {
			console.log({ res });
			if (res.data.success == 1) {
				this.setState({
					news: res.data.news
				...
				<div className=" mt-10">
					...
					<div class="well" style={{ backgroundColor: '#f5f5f5', border: '1px solid #e3e3e3' }}>
						<div class="team mt-0">
							...
							{!!matchList && !!matchList.length && <Match matchList={matchList} />}
							<div className="clearfix"></div>
							<div class="team mt-0">
