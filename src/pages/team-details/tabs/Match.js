	...
	getMatches = (id) => {
		...
		post('getTeamDetailsById', payload).then(res => {
			if ((res.data.success == 0 && res.data.error ==1) || (res.data.success == 1 && res.data.error ==0)) {
				...
				_this.props.parentCallback(res.data.team_logo,res.data.team_name,res.data.team_venuename,res.data.team_city,res.data.team_league,res.data.team_country,res.data.league_logo,res.data.country_logo);
				_this.setState({
					isLoading: false
				});
			} else {
		...
		return (
			<div class=" matches" style={{ background: "#f1f1f1", padding: "12px" }} >
				...
				{/* <div class="matches"> */}
					...
					<div class="part" style={{minWidth: 300, background: "#f1f1f1" }}>
							...
							<div class="tab-pane fade active p-3 in" id="one" aria-labelledby="one-tab">
								<div class="row mt-10" style={{ background: "#f1f1f1" }}>
									...
									{isEmpty(matchList) && !isLoading && tab===0 && <>
										<div style={{ color: '#343434', fontSize: 14, fontWeight: 700, textAlign: 'center',padding:'75px' }}>
											<div>oops !</div>
