...
const UserPlayContest = ({ predictionId, closeModal }) => {
    ...
    useEffect(() => {
        ...
        axios.post('/getcontestant', payload).then(res => {
    ...
    const getWineDetails = ((open, user_id, pred_id) => {
        ...
        axios.post('/getUserQusAnsPopup', payload).then(res => {
    ...
    return (
        <Fragment>
            ...
            >
                <div className="standing" style={{ minWidth: '300px' }}>
                    <div style={{ width: '100%' }}>
                        ...
                        <span style={{ float: 'left' }}><h2 style={{ fontSize: '14px' }}><Moment format="ddd YYYY-MM-DD HH:mm:ss">{utcToLocal(timeStamp)}</Moment></h2></span>
                                    ...
                                    return (
                                        <tr key={key} className="wpos" onClick={() => getWineDetails(true, player.user_id, player.pred_id)} >
                                            ...
                                            <td style={{ textAlign: 'center' }}>
                                                ...
                                                {player.neither != 0 && <div>None</div>}
                                                {key > 0 && <div style={{ display: "inline-block" }}>{' '}({player.neither == 0 ? (player.home_score == 0 ? player.away_score : player.home_score) : player.neither})</div>}
                                            </td>
                                        </tr>
                                    )
                                }
            ...
            <div className="standing" style={{ minWidth: '300px', height: "262px", overflow: "scroll" }}>
                    ...
                    <tbody>
                        ...
                        {!!players && players.map((player, key) => (
                            <tr key={key} className="wpos" onClick={() => getWineDetails(true, player.user_id, player.pred_id)} >
                                ...
                                { player.msisdn && (player.msisdn.trim().length > 0) ?
                                     <td >{player.msisdn.slice(-4).padStart(player.msisdn.length, 'X')}</td>
                                    :
                                     <td ></td>
                                }
