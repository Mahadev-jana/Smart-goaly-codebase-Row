...
const LiveMatchDetails = (props) => {
    ...
    const getMatches = (filterType = null) => {
        setLoading(true);
        // console.log(props.id,filterType,type)
        const payload = new FormData();
        if (props.id != 'ALL') {
            payload.append('comp_id', props.id);
        }
        if (filterType == null && type === null) {
            payload.append('type', 'yesterday');
        }if (filterType == null && type !== null) {
    ...
    return (
        <div>
            ...
            {!loading && <div className="part" style={{ marginTop: '20px' }}>
                ...
                <Link to={`/standing/${props.id}`} className="stand btn btn-default chk2" style={{backgroundColor:'#E7155D'}}>
