        ...
        <tr className="clickable-row" onClick={() => history.push(`/match/details/${id}`)}>
            <td className="text-small">
                ...
                <span style={{
                    fontSize: 20,
                    fontWeight: 'bold'
                }}>{status=='CANCL'?'CANCEL':status}</span>
            </td>
            <td>
                <div className="col-xs-6 scrL">
                    <img src={homeTeamParse.logo_path} alt="" style={{ height: '40px', width: '40px' }} />
                    {status == 'NS' | status == 'POSTP' | status == 'CANCL' | status == 'TBA' ? <span style={{ marginLeft: '15px', fontSize: 45 }}>-</span> : <span style={{ marginLeft: '15px', fontSize: 30 }}>{homeTeamParse.score}</span>}
                    <h4 className="tl mt-10" style={{ textAlign: 'center', fontSize: 11, marginLeft: 4 }}>{homeTeamParse.name}</h4>
                </div>
                <div className="col-xs-6 scrR">
                    {status == 'NS' | status == 'POSTP' | status == 'CANCL' | status == 'TBA' ? <span style={{ marginRight: '15px', fontSize: 45 }}>-</span> : <span style={{ marginRight: '15px', fontSize: 30 }}>{awayTeamParse.score}</span>}
