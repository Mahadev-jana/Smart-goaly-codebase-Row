...
const FavoriteClubModal = ({ closeModal }) => {
    ...
    const saveFavoriteClubs = e => {
        if (length == selectedClubs.length) {
            ...
            const payload = new FormData();
            const userdetails = JSON.parse(localStorage.getItem('userDetails'));
            // console.log('userid',selectedClubs.id)
            // console.log('userid',selectedClubs.name)

            payload.append('clubs', JSON.stringify(selectedClubs));
            // payload.append('league_details', JSON.stringify(allclubs));
            payload.append('user_id', (userdetails.id));
            axios.post('/UserFavouriteTeam', payload).then(res => {
        ...
        <>
            ...
            <div style={{ padding: '10px 10px 25px 10px' }}>
                ...
                {!isLoading && allclubs && <div style={{
                    ...
                    <button onClick={saveFavoriteClubs} className="pull-right" style={{ width: '100%', background: '#159B3E', marginBottom: 5 }}>SAVE</button>
                    <button className="pull-right" style={{ width: '100%', background: '#A8D5B3', marginBottom: 15 }} onClick={closeModal}>Close</button>
                </div>}
            </div>
